# DESIGN — Attention Cascade

The technical design: contracts, prompts, thresholds and the reasoning behind each. Where this
document gives a function signature, a schema or a prompt, that is the version the code
implements verbatim.
...
**Success is a measured comparison table, two live failure demos, and an audit trail.**
...
```
attention-cascade/
├── README.md                     # the measured results and how to run them
├── DESIGN.md                     # this file
├── DECISIONS.md                  # architectural decisions and their rejected alternatives
├── FILE_TOUR.md                  # one line per source file
├── LICENSE
├── pyproject.toml
├── uv.lock
├── .env                          # gitignored
...
├── runs/
│   ├── cache/                    # LLM response cache, committed for offline runs
...
│   ├── __init__.py
│   ├── config.py                 # constants, pricing, thresholds
│   ├── models.py                 # pydantic types
│   ├── blackboard.py             # SQLite shared store
│   ├── generator.py              # synthetic streams + incidents
│   ├── enrich.py                 # blind text enrichment
│   ├── groundtruth.py            # QUARANTINED accessors
│   ├── llm.py                    # metered, cached Vertex client
│   ├── detectors.py              # Tier 0
│   ├── triage.py                 # Tier 1
│   ├── correlate.py              # Tier 2
│   ├── gate.py                   # sufficiency + 7-cap
│   ├── orchestrator.py           # asyncio workers, queues, shedding
│   ├── baseline.py               # naive all-events-to-frontier
│   ├── metering.py               # cost accounting
│   ├── report.py                 # the table + recall waterfall
│   ├── chaos.py                  # kill / flood injectors
│   └── cli.py                    # typer CLI, entry point `ac`
└── tests/
    ├── test_generator.py
    ├── test_gate.py
    ├── test_blackboard.py
    └── test_no_groundtruth_leak.py   # see section 11
...
This is the answer to "did you build the test you pass": the structure is planted, the prose is
blind, and the near-misses are prose-identical to the real incidents.

INC-5 is deliberately a *silence* signal — the detector must catch absence of activity, not a
spike. It is the most interesting one to demonstrate.

**Note on silence:** an absence has no events of its own. The `ticket_silence` detector sets
...
Target output: **60–100 anomalies from ~1000 events.** Below 40 or above 150, the thresholds in
`config.py` need retuning — on *noise density* only, never by looking at ground truth.
...
- **Thinking budget 0.** This tier answers a yes/no question; internal reasoning here is pure
  ...
  cheap tier and on in the expensive tier is itself an attention-budget decision.
...
**A batch returning malformed JSON is retried once with a repair instruction, then fails open** —
every anomaly in that batch is marked `plausible=True` and the fail-open is written to the audit
trail. Failing open is correct here: a triage error must not silently destroy recall.
...
**Every cited `event_id` is validated against the blackboard. Hallucinated ids are dropped and
counted** — hallucination rate is a number worth reporting.

---

## 8. The attention gate (zero tokens, deliberately not a model)

`gate.py`. The heart of the design. Pure function, fully unit-tested.
...
Every rejection and every displacement is written to the audit trail with its reason, so any
near-miss can be traced to the exact rule that rejected it.
...
- Model: identical to Tier 2, with the same thinking budget of 2048. A baseline given a weaker
  model or less thinking would rig the comparison.
...
- **The long-context cliff.** If the single call crosses 200K input tokens, Pro rates roughly
  double to $4/$18. This is not avoided — it is measured and reported. `config.crossed_long_context()`
...
The report states the chunking explicitly if it triggers.
...
|---|---|---|
| Method | all events → frontier model | Tier 0 → Tier 1 → Tier 2 |
| LLM calls | | |
| Input tokens | | |
...
**Table B — the recall waterfall.** The table that localises where recall is lost:
...
tagged with that `incident_id` from ≥2 distinct streams. `report.py` implements exactly this definition and the README states it — a vague recall
definition is the first thing worth attacking.

**Cost formula:** `config.price_call(model, input_tokens, output_tokens, thoughts_tokens,
cached_input_tokens)` is the single implementation. It handles thinking tokens billing at the
...
`thoughts_token_count`, `cached_content_token_count`. Never estimated, never len(text)/4.
...
"here is a number" and "here is a measurement". Variance across seeds is a finding about the
architecture, not an embarrassment, and is reported as such.

**Table C — cost by tier.** One row per tier: calls, input, visible output, thinking tokens, cost,
and share of total spend. The expected shape is Tier 2 holding most of the cost across a
small number of calls. Any other shape means the batching is wrong.

---

## 11. Ground-truth quarantine (mandatory test)

`tests/test_no_groundtruth_leak.py`:
...
This test failing is a build-stopping error.
...
- SQLite: `PRAGMA journal_mode=WAL`, one write lock, `busy_timeout=5000`. All writes go through
  `Blackboard`, which serializes them behind a single `asyncio.Lock`. Writes are serialized
  deliberately so evidence cannot double-count.
...
`llm.py` is the single gateway. Its contract:
...
**`runs/cache/` is committed to git.** That is what makes a run work with no network.

**No other module may import `google.genai`.** A client constructed outside `llm.py` produces
calls that are not metered, and the cost table would be wrong. `tests/test_no_groundtruth_leak.py`
enforces this with an AST walk.
...
uv run ac check-linkage                         # assert every incident is joinable across streams
```

`ac demo` prints a live progress line showing the collapse: `1000 events → 84 anomalies →
...
| Model id not found in region | `ac verify --list-models` prints what is available and fails with that list. A different model is never silently substituted. |
...
**No exception is swallowed silently.** Every caught exception writes to the audit trail.
...
- Every module carries a 3-line docstring at the top: what it does, what it does not do, why it
  exists.
...
## 17. Out of scope — judgement, not gaps
